...
impl DirectoryIterator {
    fn new(path: &str) -> Result<DirectoryIterator, String> {
        ...
        return match unsafe { opendir(path.as_ptr() as *const i8) } {
            ptr if !ptr.is_null() => Ok(DirectoryIterator {
                path: CString::new(path).unwrap(),
                dir: ptr,
            }),
            _ => Err(format!("Could not open directory {}", path)),
        };
    }
}
